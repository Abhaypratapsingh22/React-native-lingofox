...
# Create a test user (preferred; curated flags). Keep the password out of history and arguments.
read -r -s -p 'Test-only password: ' TEST_PASSWORD; printf '\n'
tmpdir=$(mktemp -d); chmod 700 "$tmpdir"; trap 'rm -rf "$tmpdir"' EXIT
clerk users create --email alice@example.com --password "$TEST_PASSWORD" \
  --first-name Alice --last-name Doe --dry-run
# Review the preview, obtain explicit approval, then run the same command without --dry-run.
clerk users create --email alice@example.com --password "$TEST_PASSWORD" \
  --first-name Alice --last-name Doe

# Equivalent raw BAPI call for test credentials. Use only when curated flags don't cover a field.
...
clerk config patch --json '{"session":{"lifetime":3600}}' --instance prod --dry-run
# After reviewing the production preview and obtaining explicit approval:
clerk config patch --json '{"session":{"lifetime":3600}}' --instance prod
...
# Target a verified ignored local file
...
```sh
# Persist once in a private directory, query as many times as you need, then clean up.
tmpdir=$(mktemp -d); chmod 700 "$tmpdir"; trap 'rm -rf "$tmpdir"' EXIT
clerk users list --json --limit 250 > "$tmpdir/users.json"

jq '.data | length'                         "$tmpdir/users.json"   # count rows on the page
jq '.hasMore'                               "$tmpdir/users.json"   # any more pages?
jq '.data[0] | keys'                        "$tmpdir/users.json"   # learn the shape of one record
jq '.data[] | {id, first_name, last_name}'  "$tmpdir/users.json"   # project non-sensitive fields
```
...
```sh
# Always --dry-run first across the whole set. Process every page before applying patches.
offset=0
tmpdir=$(mktemp -d); chmod 700 "$tmpdir"; trap 'rm -rf "$tmpdir"' EXIT
while :; do
  page="$tmpdir/users-${offset}.json"

...
Before downloading and executing any unpinned package-runner version, obtain explicit user approval. Prefer a globally installed, verified binary.
...
Before running commands that require authentication or a linked project, verify the CLI is authenticated, linked, and healthy:
...
Run `clerk doctor --json` for those authenticated/link-dependent commands. Keyless bootstrap (`clerk init`) and instance-command flows can proceed without this preflight; use the separate keyless path below.
...
## Keyless: no login required

`clerk init` does **not** require `clerk auth login`: on a keyless-capable framework an unauthenticated run mints a **keyless** app with temporary dev keys — no account, no browser, no flag. Most instance commands then work on that key alone.

Two things to know before you use it: keyless follows whatever `sk_` key is local — `sk_live_` included, claimed or not — so pass `--app <id>` when you mean a real app; and `clerk open` returns a credential-equivalent claim URL, never safe to paste into a log or PR.

Which commands need an account, and the full rules: [auth.md](references/auth.md#keyless-operating-without-an-account).
...
clerk api /users/user_abc123 -X DELETE            # agent mode executes after explicit approval
...
**Always preview a mutation with `--dry-run`, display the target and preview, and obtain explicit user approval before running it.** Then re-run without `--dry-run`. Agent-mode mutations generally omit `--yes`; `clerk unlink` is the exception and requires `--yes` to avoid a usage error. The flag is not human approval.
...
```sh
# 1. Persist the response in a private directory and remove it on exit.
tmpdir=$(mktemp -d)
chmod 700 "$tmpdir"
trap 'rm -rf "$tmpdir"' EXIT
clerk users list --json --limit 250 > "$tmpdir/users.json"
clerk apps list --json                > "$tmpdir/apps.json"
clerk api /users/user_abc123          > "$tmpdir/user.json"

# 2. Inspect only what you need.
jq '.data | length'                       "$tmpdir/users.json"   # current page size
jq '.hasMore'                             "$tmpdir/users.json"   # are more pages available?
jq '.data[0] | keys'                      "$tmpdir/users.json"   # discover the user shape once
jq '.data[] | {id, first_name, last_name}' "$tmpdir/users.json" # project to non-sensitive fields
jq '[.data[] | select(.banned)] | length' "$tmpdir/users.json"   # aggregate without reading rows
```

**If `jq` is not available**, fall back to Python or Node - both can stream the file without printing it whole:

```sh
python3 -c 'import json; d=json.load(open("'"$tmpdir"'/users.json")); print(len(d["data"]), d["hasMore"])'
node -e 'const d=require(process.argv[1]); console.log(d.data.length, d.hasMore)' "$tmpdir/users.json"
```
...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `clerk init`                  | Scaffold Clerk into a project. Keyless by default when unauthenticated on a keyless-capable framework — no login required (see [Keyless](#keyless-no-login-required)). `--starter` only supports bootstrap for Next.js, React Router, Astro, Nuxt, TanStack Start, React, Vue, and JavaScript. `--keyless` with `--app`/`--login`, or `--template`/`--fresh` on any non-keyless run (including while merely signed in), is a usage error — and it fires *after* scaffolding, so a starter run leaves the project behind. | `--framework`, `--pm`, `--name` (with `--starter`), `--app`, `--starter`, `--keyless`, `--login`, `--template`, `--fresh`, `-y`, `--no-skills`                                   |
| `clerk auth login`            | OAuth browser login (stores token). Not required for keyless bootstrap or keyless instance config. Agent mode: no-op if already logged in. With no stored session it still opens a browser and binds a localhost callback, so it is not unattended; prefer `CLERK_PLATFORM_API_KEY` for headless flows. Aliases: `signup`, `signin`, `sign-in`. Top-level shortcut: `clerk login`. | -                                                                                                                                                                                |
...
- **`init` is keyless by default — do not log in first.** An unauthenticated agent run on a keyless-capable framework mints a keyless app: no flag, no account, no browser. `--app <id>` or a pre-link targets a real app instead; `--keyless` forces keyless over both a session and an existing link. Only a non-keyless framework with no app target prints manual guidance and exits cleanly. Flag exclusivity is in the command table above.
- **`--fresh` is destructive.** It replaces the keyless app and overwrites the env keys and `.clerk/keyless.json` with no prompt, orphaning the previous app and its users. Never pass it just to re-run `init`.

...
1. **Check CLERK_SECRET_KEY** — verify it is non-empty without printing any part of it:
   ```bash
  [[ -n "${CLERK_SECRET_KEY:-}" ]] && echo "CLERK_SECRET_KEY is set" || echo "CLERK_SECRET_KEY is empty"
   ```
...
Before executing either POST, display the organization name, invitee email, and role, then wait for explicit user confirmation. POST operations are not immediate; only non-write operations may execute without confirmation.

```bash
# Step 1 — Create organization (run only after explicit confirmation)
...
Metadata updates deep-merge the existing metadata. For replacement, use `PUT /v1/users/${USER_ID}/metadata` or `clerkClient.users.replaceUserMetadata(userId, ...)`: each supplied top-level metadata field is replaced in full, omitted top-level fields remain unchanged, and an empty object clears the supplied metadata field.
...
```bash
curl -s "https://api.clerk.com/v1/users?limit=100&offset=0&order_by=-created_at&created_at_after=$(( $(date -d '7 days ago' +%s 2>/dev/null || date -v-7d +%s) * 1000 ))" \
  -H "Authorization: Bearer $CLERK_SECRET_KEY" \
  | python3 -c "
import sys, json
...
Deep-merges the existing metadata.
PUT /v1/users/{user_id}/metadata
Body (JSON, snake_case): { public_metadata, private_metadata, unsafe_metadata }
Replaces the metadata completely.
```
...
### Metadata Updates (Deep Merge or Replacement)

`updateUserMetadata({ publicMetadata: { role: 'admin' } })` deep-merges public metadata. Use `replaceUserMetadata()` or `PUT /v1/users/{user_id}/metadata` when the complete metadata object should be replaced.

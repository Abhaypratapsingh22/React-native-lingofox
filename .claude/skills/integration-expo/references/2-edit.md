...
Where possible, add calls for PostHog's identify() function on the client side only after successful authentication and session creation. Use the canonical authenticated user ID from the auth system (e.g. Clerk's `user.id`) as the distinct ID — never use raw form values such as email addresses or passwords as the distinct ID or as PostHog properties. If there is server-side code, pass the authenticated distinct ID to the server-side code to identify the user. On the server side, make sure events have a matching distinct ID where relevant. 

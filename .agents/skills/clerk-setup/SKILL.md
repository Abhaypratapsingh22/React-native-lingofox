...
**No login required.** Unauthenticated, `clerk init` on a keyless-capable framework (Next.js, Astro, Nuxt, TanStack Start, React Router) mints a temporary **keyless** app — no account, no browser, no flag. Don't run `clerk auth login` first. Authenticated (or with `--app` / `--login`) it creates and links a real app via PLAPI instead.
...
- Keyless projects are configurable without an account — see the [keyless command table](../clerk-cli/references/auth.md#keyless-operating-without-an-account) for which commands work and which need a claimed app.
...
> **shadcn/ui detected** (`components.json` exists): Explain the optional shadcn theme integration and ask for explicit user approval before installing or using `@clerk/ui`. Without approval, do not add or use the dependency.
...
If the project uses shadcn/ui (check for `components.json` in the project root), offer the optional shadcn theme so Clerk components can match the app's design system. Require explicit user approval before installing or using `@clerk/ui`; without approval, explain the option and leave dependencies unchanged:
